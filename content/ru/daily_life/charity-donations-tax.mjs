  ...
  steps: [
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          '🇵🇹 Также: можно consignar benefício fiscal do IVA — передать благотворительной организации сумму вашего вычета по IVA из подходящих счетов (faturas). Это НЕ бесплатно: в отличие от 1% IRS, вы теряете этот вычет по IVA'
        ]}
      ]
    },
    {
        ...
        { kind: 'substeps', items: [
          { id: 'd1', title: 'Условия', content: [
            { kind: 'checklist', items: [
              ...
              '✅ Вычет: обычно 25% от суммы пожертвования; для некоторых пожертвований в рамках социального меценатства сумма может быть увеличена (например, до 130%), но только если получатель и пожертвование соответствуют требованиям EBF (статьи 61-66)'
            ]}
          ]},
        ...
        { kind: 'substeps', items: [
          ...
          { id: 'c3', title: 'Cruz Vermelha (Португальский Красный Крест)', content: [
            { kind: 'checklist', items: [
              ...
              'Волонтёрские программы в 200+ отделениях (delegações)',
    ...
    },
    {
      ...
      content: [
        { kind: 'checklist', items: [
          '🎯 European Solidarity Corps (вместо устаревшего EVS): волонтёрские проекты в Португалии и ЕС, с компенсацией проезда, жильём и питанием или стипендией в зависимости от проекта',
          '🎯 Кампании Banco Alimentar: 2 раза в год, 4-8 часов, очень простой вход',
          '🎯 Волонтёрство с детьми: через аккредитованные IPSS и ассоциации, с проверкой кандидата и его прошлого. CPCJ — официальные комиссии защиты детей; обращайтесь к ним за консультацией, а не как к месту для обычного волонтёрства',
