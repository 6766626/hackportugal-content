export default {
  ...
  tldr: 'С 01.07.2021 отменён беспошлинный порог 22 € — теперь IVA облагается любая посылка из стран вне ЕС, а таможенная пошлина добавляется, если стоимость выше 150 €. Оформление идёт через получателя: CTT или курьер (DHL, FedEx, UPS) присылают SMS со ссылкой на оплату. Оплата онлайн: карта, MB WAY, Multibanco. Сбор CTT за растаможку зависит от продукта; у курьеров — 10–40 €. Актуальные тарифы — на ctt.pt.',
  ...
  steps: [
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          '⏳ На горизонте: ЕС согласовал отмену беспошлинного порога 150 € для интернет-посылок. Переходные меры (включая сбор за обработку) ожидаются в течение 2026 года, но на май 2026 порог 150 € ещё действует'
        ]}
      ]
    },
    {
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Через Portal Aduaneiro на portaldasfinancas.gov.pt → «Alfândegas». Declaração H7 — упрощённая декларация для недорогих посылок до 150 €, не облагаемых пошлиной; она не подходит для подакцизных товаров и товаров с ограничениями.' },
        { kind: 'checklist', items: [
      ...
      content: [
        { kind: 'substeps', items: [
          ...
          { id: 'q2', title: 'AliExpress / Shein — они платят IVA?', content: [
            { kind: 'paragraph', text: 'IOSS (Import One-Stop Shop) применяется к товарам собственной стоимостью до 150 € и не действует на подакцизные товары. Если маркетплейс зарегистрирован в IOSS, IVA взимается в момент покупки. Видите в чеке «VAT included» — всё в порядке. Если нет — CTT уведомит при импорте.' }
          ]},
