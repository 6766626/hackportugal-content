export default {
  ...
  steps: [
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Если в стране нет палаты медсестёр: запросите документ у компетентного госоргана/регулятора здравоохранения об отсутствии запрета/санкций; справка работодателя может быть только дополнением, её приемлемость уточните в OE.',
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          'Диплом о высшем сестринском образовании, сопоставимый с португальской Licenciatura em Enfermagem (4 года, не менее ~4600 часов, половина — клиническая практика). Если у вас только среднее профессиональное/колледжное сестринское образование, заранее уточните в вузе: его могут не признать достаточным для титула enfermeiro/enfermeira.',
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Для общения с пациентами, врачами и заполнения clinical records OE требует владение португальским. По действующему регламенту Prova de Comunicação Linguística (Regulamento 339/2017) сертификат должен соответствовать как минимум уровню C1 QECR. Признаются CAPLE DAPLE и DUPLE; OE может принять и другой документ, если он отвечает требованиям процедуры.' },
        { kind: 'checklist', items: [
        ...
        { kind: 'warning', text: 'Уровень B2 ниже формального минимума OE (C1): без аттестации Prova de Comunicação Linguística inscrição не оформят. Если в конкретном случае OE принимает иной документ, это исключение нужно подтверждать письменно до подачи.' }
      ]
    },
    {
      id: 'oe-registration',
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Условный contrato/promessa de contrato для визы можно подписать и до cédula, но начинать работу и выполнять функции именно как enfermeiro/enfermeira — только после активной inscrição/cédula OE.'
        ] },
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'warning', text: 'Иммиграционная часть отдельно: OE не выдаёт визу и ВНЖ. Если вы переезжаете из-за рубежа для работы в португальской клинике, обычно смотрят визу D1 (при трудовом договоре или обещании договора) или D3 (если позиция и квалификация проходят как atividade altamente qualificada). Виза D8 — только для удалённой работы на иностранного работодателя/клиентов и для трудоустройства медсестрой в Португалии не подходит. С 23.10.2025 действует Lei 61/2025, поэтому “въехать туристом и легализоваться позже” больше не является надёжной стратегией.' }
      ]
    }
  ],
  costs: [
  ...
  sources: [
    ...
    {
      title: 'Ordem dos Enfermeiros — Normas para implementação de Prova de Comunicação Linguística (уровень C1)',
