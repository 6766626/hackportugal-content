export default {
  ...
  steps: [
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Для наёмных работников нужен índice de profissionalidade: минимум 12 дней с оплатой в первые 4 месяца из последних 6 перед болезнью'
        ] }
      ]
    },
    {
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Если болезнь продолжается после autodeclaração, нужен врач и CIT на последующий период; первые 3 дня обычно остаются неоплачиваемыми, а дату начала CIT определяет врач по медицинской оценке, а не по желанию работника'
        ] },
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Размер считается от remuneração de referência — дневной базы, рассчитанной по зарегистрированным зарплатам в Segurança Social. Упрощённо: берут заработки за первые 6 из 8 месяцев перед месяцем начала болезни и делят на 180. Subsídio de férias и subsídio de Natal в этот расчёт, как правило, не входят, поэтому база Segurança Social обычно ниже вашей месячной зарплаты и тем более «зарплаты на руки».' },
        { kind: 'checklist', items: [
          'До 30 дней болезни: 55% remuneração de referência',
          'С 31-го по 90-й день: 60%',
          'С 91-го по 365-й день: 70%',
          'С 366-го дня: 75%',
          'Только ставки 55% и 60% могут вырасти до 60% и 65%: если remuneração de referência ≤ 500 € либо в семье 3+ детей до 16 лет (или до 24 с abono de família), либо ребёнок с надбавкой по инвалидности. К ставкам 70% и 75% это не применяется',
          'Есть минимум и потолок: дневное пособие не ниже 30% дневного IAS (в 2026 это около 5,37 € в день), но не больше чистой remuneração de referência; точную сумму смотрите в Segurança Social Direta',
    ...
    },
    {
      ...
      content: [
        { kind: 'checklist', items: [
          ...
          'Во время оплачиваемой baixa не работайте, включая удалёнку и фриланс: CIT удостоверяет нетрудоспособность, а не разрешает работу из дома. Нарушение грозит возвратом пособия и дисциплинарными последствиями',
  ...
  costs: [
    ...
    { label: 'Приём в SNS для получения CIT', amountEUR: 0, note: 'В первичном звене SNS (centro de saúde) обычно бесплатно; в urgência hospitalar возможна taxa moderadora без направления; частная консультация — по тарифу клиники/страховки' },
