export default {
  ...
  tldr: 'Чтобы работать врачом в Португалии, иностранного диплома недостаточно: Уточнить: «Для дипломов/квалификаций из-за пределов EU/EEA/Switzerland обычно сначала требуется академическое признание Medicina по DL 66/2018 через DGES/португальский вуз. Для EU/EEA/Swiss medical qualifications может применяться отдельный режим профессионального признания по Lei 9/2009/Directive 2005/36/EC — проверяйте требования OM для этой категории».. Базовый сбор за заявку — 385 €. OM проверяет документы, Good Standing и, как правило, требует Prova de Comunicação Médica. Для полной автономной регистрации важен стаж: если после диплома нет минимум 2 лет врачебной практики, могут направить на formação geral/практический путь до автономии.',
  ...
  steps: [
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Ordem dos Médicos — обязательная профессиональная палата для врачей в Португалии. Уточнить: «Для медицинской практики в Португалии, включая telemedicina для пациентов/провайдеров в португальской системе, нужна активная inscrição в OM. Для cross-border telemedicine из Португалии также отдельно проверяйте право страны пациента/работодателя, страховку malpractice и португальские правила».' },
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Заменить на: «Для врачей с дипломом, полученным вне EU/EEA/Switzerland, стандартный путь обычно начинается с академического reconhecimento по Decreto-Lei 66/2018; для Medicina чаще всего требуется reconhecimento específico. Если квалификация получена в EU/EEA/Switzerland, сначала проверьте route profissional/automatic recognition по Lei 9/2009 у OM».: португальский вуз с медицинским факультетом сравнивает вашу программу с Mestrado Integrado em Medicina.' },
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: ['Считайте стаж после даты получения диплома Medicina, а не годы учёбы', 'Internship/residency может засчитываться, если это была врачебная клиническая работа', 'Волонтёрство без статуса врача обычно слабое доказательство', 'Справка должна быть конкретной: specialty/department, dates, workload, clinical duties', 'Если работали в нескольких странах, возьмите Good Standing и стажевые справки из каждой', 'Вынести отдельным блоком в начало гайда: «EU/EEA/Swiss route: если базовая медицинская квалификация получена в EU/EEA/Switzerland и соответствует Directive 2005/36/EC, процедура может идти как профессиональное признание через OM/компетентный орган, с документами о conformity, good standing и праве практиковать, а не как обычное DGES reconhecimento específico».'] }
      ]
    },
    {
      id: 'submit-in-person',
