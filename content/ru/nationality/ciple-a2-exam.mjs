export default {
  ...
  tldr: 'Сертификат уровня A2 нужен только для натурализации по проживанию. Реформа Lei da Nacionalidade действует с 19.05.2026: срок проживания увеличен с 5 до 7 лет для CPLP/EU и 10 лет для остальных (заявления, поданные до 19.05.2026, рассматриваются по старым правилам — 5 лет). Закон также вводит тест на знание языка, культуры, истории и устройства государства, но он станет обязательным лишь после публикации отдельного регламента. Для брака, união de facto, Reagrupamento Familiar — НЕ нужен.\n\nВАЖНО: для натурализации IRN принимает только официальный сертификат уровня A2+ (CIPLE / PLA / Centros Qualifica / DEPLE / DAPLE) — простая справка о посещении 150 часов курса БЕЗ оценки уровня НЕ принимается. Самые дешёвые пути: PLA (0 €), IEFP (0 € + стипендия, но с обязательствами как безработного), государственные школы для взрослых ~20 €/семестр. Экзамен CIPLE — 85 € (2026), ежемесячные сессии.',
  ...
  steps: [
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          '✅ Натурализация по проживанию (art. 6 Lei 37/81 + art. 25 Decreto-Lei 237-A/2006): по действующему праву — после 5 лет. Реформа Lei da Nacionalidade (промульгирована 03.05.2026, опубликована в DR 18.05.2026, действует с 19.05.2026) увеличила срок до 7 лет для CPLP/EU и 10 лет для остальных. Заявления, поданные и находившиеся на рассмотрении до 19.05.2026, рассматриваются по старым правилам (5 лет). Закон также вводит тест на знание португальского языка, культуры, истории, символов, прав и обязанностей и устройства государства, но он станет обязательным только после публикации отдельного регламента (regulamentação)',
          '✅ Для приёма в ряд программ IEFP, государственных колледжей и Centros Qualifica'
        ]},
        { kind: 'paragraph', text: 'Когда сертификат CIPLE НЕ требуется:' },
        { kind: 'checklist', items: [
          '❌ Гражданство по браку / união de facto — Lei 37/81 язык не требует (только «эффективные связи»)',
          '⚠️ Гражданство по происхождению — внуки/правнуки португальцев (Lei 37/81 art. 1(1)(d) и art. 1(1)(e)): закон требует «sufficient knowledge of the Portuguese language» (достаточное знание португальского), а не конкретно A2. Сертификат CIPLE A2 или эквивалент — самый надёжный способ подтвердить; в спорных случаях IRN может также принимать собеседование, школьные документы из PT-говорящей страны, документы CPLP и др. Дети португальцев (art. 1(1)(c)) — обычно без языкового требования',
        ...
        { kind: 'warning', text: 'При подаче на натурализацию по проживанию (art. 6) IRN принимает только ОФИЦИАЛЬНЫЙ сертификат, в котором указан конкретный уровень A2 или выше. Справка о посещении курса (frequência) без оценки уровня — НЕ подходит. Для внуков/правнуков (art. 1(1)(d/e)) набор приемлемых доказательств шире — уточняйте в IRN, если CIPLE недоступен.' }
      ]
    },
    {
