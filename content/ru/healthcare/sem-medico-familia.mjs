export default {
  ...
  tldr: 'Отсутствие médico de família не лишает вас доступа к SNS. Если есть número de utente, обращайтесь в свой Centro de Saúde/USF/UCSP за consulta aberta, consulta de recurso или atendimento complementar; при симптомах звоните SNS 24 — 808 24 24 24, чтобы получить триаж и направление. С середины июня 2026 вводится практическая чистка списков: пациенты, не контактировавшие с SNS 5 лет, могут потерять закреплённого врача, но не регистрацию в SNS.',
  ...
  steps: [
      ...
      content: [
        { kind: 'substeps', items: [
          ...
          { id: 'sns24', title: '2. Позвонить SNS 24 при симптомах', content: [
            ...
            { kind: 'paragraph', text: 'Для иностранца это важно: направление от SNS 24 снижает риск напрасного ожидания в urgência и показывает системе, что вы обратились не вслепую, а по медицинскому маршруту.' }
          ] },
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'С середины июня 2026 в cuidados de saúde primários запускается механизм актуализации списков: пациенты, которые 5 лет не имели контактов с SNS, могут потерять закреплённого médico de família. Раньше учитывались только обращения в первичное звено, теперь — любой контакт с системой SNS. Цель — освободить места у врачей для людей, которые реально живут в районе и нуждаются в наблюдении.' },
        { kind: 'paragraph', text: 'Это не означает потерю número de utente или права на SNS. Речь о списке конкретного семейного врача. Но для экспатов риск практический: если вы уехали, сменили адрес, не реагируете на запросы системы или ни разу не обращались, вас могут счесть неактивным.' },
        { kind: 'checklist', items: [
