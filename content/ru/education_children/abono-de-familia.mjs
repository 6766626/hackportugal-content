export default {
  ...
  steps: [
    ...
    },
    {
      ...
      title: 'Поддержание',
      content: [
        { kind: 'paragraph', text: 'Эшелон пересчитывается автоматически после подачи IRS (август-сентябрь). Если IRS не подавался/нет налоговых данных или семья недавно переехала, Segurança Social может запросить prova de rendimentos/condição de recursos. Для детей/молодёжи старше 16 лет ежегодно подтверждайте обучение в SSD в срок, указанный SS (prova escolar).' },
