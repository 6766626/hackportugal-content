export default {
  ...
  tldr: 'Государство субсидирует покупку BEV через Fundo Ambiental — суммы, лимиты, потолок цены и квоты определяются ежегодным **Aviso Fundo Ambiental**. Прошлые Avisos: для физических лиц BEV M1 — около € 4 000 (обычно не более 1 машины на кандидата), для бизнеса N1 — около € 6 000; для M1-компаний условия отличаются. Бюджет ограничен и быстро исчерпывается. **ISV**: 0% для чистых BEV. **IUC**: BEV обычно exempt (нет CC-компоненты у чистых EV); PHEV/HEV — по применимой категории IUC. Зарядка Mobi.E — €0.28–0.55/кВт·ч. До покупки проверяйте текущий Aviso на fundoambiental.pt.',
