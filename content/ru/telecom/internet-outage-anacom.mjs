export default {
  ...
  tldr: 'Если fibra/интернет в Португалии не работает, сначала фиксируйте avaria у оператора и берите número de avaria/protocolo. По Lei 16/2022 при недоступности услуги более 24 часов по вине оператора положена компенсация минимум пропорционально mensalidade. Если неисправность длится более 15 дней после сообщения, потребитель может расторгнуть договор без fidelização/штрафа. ANACOM принимает жалобы и осуществляет надзор, но деньги обычно возвращают через Livro de Reclamações и consumer arbitration.',
  ...
  steps: [
    ...
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          'Если услуга недоступна более 24 часов — подряд или суммарно за расчётный период — по причинам, относимым к оператору, клиент имеет право на компенсацию.',
        ...
        { kind: 'warning', text: '15 дней безопаснее отсчитывать от официально зарегистрированной avaria, а не от первого устного разговора «у меня плохо ловит». Поэтому номер заявки критичен.' }
      ]
    },
    {
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Если услуга недоступна более 15 дней после сообщения avaria, не ограничивайтесь тем, чтобы «просто перестать пользоваться». Отправьте formal pedido de resolução. Иначе оператор может считать договор активным и продолжать выставлять faturas.' },
        { kind: 'checklist', items: [
        ...
        { kind: 'warning', text: 'Если в pacote включены мобильные номера, оператор может попытаться оставить мобильную часть или изменить цену. В заявлении укажите, расторгаете ли вы весь pacote или только фиксированную услугу, и просите расчёт новой цены до согласия.' }
      ]
    },
    {
      id: 'arbitration',
