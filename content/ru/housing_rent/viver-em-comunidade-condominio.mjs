  ...
  steps: [
    {
      ...
      content: [
        ...
        { kind: 'warning', text: 'Если вы снимаете квартиру, не верьте «condomínio incluído» на словах. В договоре аренды должно быть ясно: кто платит ежемесячные quotas, кто платит внеплановые работы (obras extraordinárias) и включены ли гараж, лифт, бассейн, кладовка.' }
      ]
    },
    {
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Деньги резерва идут на работы по сохранению здания: крыша, фасад, стояки, лифт, структурный ремонт.',
          'Дополнительные quotas возможны, если резерв не покрывает работы: например, ремонт фасада на 40 000 € делят между condóminos по permilagem.',
        ...
        { kind: 'warning', text: 'Низкая quota не всегда плюс. Если дом годами собирал по 10–15 € и не формировал резерв, новый собственник может быстро получить дополнительную quota на крышу, лифт или фасад в тысячи евро.' }
      ]
    },
    {
      id: 'assembleia',
      ...
      content: [
        { kind: 'paragraph', text: 'Обычная ежегодная assembleia собирается для утверждения счетов прошлого года и бюджета нового года. По Código Civil art. 1431 она проводится в первой половине января, но regulamento или решение condóminos может перенести обычное собрание на первый квартал.' },
        { kind: 'checklist', items: [
        ...
        { kind: 'paragraph', text: 'Для экспата практический минимум — узнать контакты administrador, дату следующей assembleia, размер ежемесячной quota, saldo do fundo comum de reserva и есть ли утверждённые работы на ближайшие 12–24 месяца.' }
      ]
    },
    {
      id: 'regulamento',
      ...
      content: [
        ...
        { kind: 'checklist', items: [
          ...
          'Гараж: нельзя автоматически использовать машино-место как склад, мастерскую или зарядку для электромобиля без соблюдения правил безопасности.',
        ...
        { kind: 'substeps', items: [
          { id: 'tenant', title: 'Если вы арендуете', content: [
            { kind: 'checklist', items: [
              ...
              'Дополнительные quotas на фасад, крышу, лифт и структурные работы обычно должны оставаться за собственником.',
              'Попросите regulamento до въезда, особенно если есть собака, велосипед, гараж, кондиционер или планы ремонта.',
              'Уточните правила шума, пользования лифтом при переезде, вывоза мебели и использования garagem/arrecadação.',
          ...
          { id: 'buyer', title: 'Если вы покупаете', content: [
            { kind: 'checklist', items: [
              ...
              'Проверьте ежемесячную quota и saldo do fundo comum de reserva.',
              'Ищите в atas слова obras, infiltrações, fachada, telhado, elevador, garagem, dívida.',
              'Сравните состояние здания с размером резерва: старый дом + маленький фонд = риск дополнительной quota.',
              'Проверьте, есть ли профессиональный administrador или всё держится на одном соседе-добровольце.',
              'Если покупаете верхний этаж или подвал, отдельно проверяйте протечки крыши, terraço, humidade и responsabilidades comuns.'
            ] }
          ] }
        ] }
      ]
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Типичная цепочка: сначала пишете administrador с фактами, фото, датами и ссылкой на regulamento; затем вопрос выносится на assembleia; если не решается — жалоба в Câmara Municipal, полиция при шуме или опасности, либо гражданский иск.' },
        { kind: 'checklist', items: [
        ...
        { kind: 'warning', text: 'Не удерживайте арендную плату или взнос в condomínio самовольно, без юридической консультации. В Португалии это часто ухудшает позицию: сначала письменное уведомление, доказательства, prazo para resolver, затем официальная процедура.' }
      ]
    }
  ],
  costs: [
    { label: 'Обычные ежемесячные quotas', amountEURMin: 20, amountEURMax: 100, note: 'Типичный диапазон для квартиры в обычном доме; не законный тариф, зависит от permilagem, лифта, гаража, услуг и города.' },
