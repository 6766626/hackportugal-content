  ...
  steps: [
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Autoliquidação / reverse charge означает: вы не добавляете Portuguese IVA в счёт, а клиент сам начисляет VAT в своей стране. Для типичного фрилансера в Португалии это применяется к услугам B2B для клиента из другой страны ЕС, если клиент является taxable person и дал действительный VAT ID.' },
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'VIES — официальный сервис Еврокомиссии для проверки VAT ID компаний ЕС. Для reverse charge это ключевое доказательство, что клиент действительно B2B. Проверяйте VAT ID перед первым счётом каждому клиенту и затем регулярно, особенно если сотрудничество длится месяцами.' },
        { kind: 'checklist', items: [
        ...
        { kind: 'warning', text: 'Результат «valid» в VIES не делает частное лицо бизнес-клиентом автоматически. Но отсутствие действительного VAT ID почти всегда ломает B2B reverse charge для ЕС: Finanças может потребовать Portuguese IVA, juros compensatórios и штраф.' }
      ]
    },
    {
      id: 'invoice-text',
    ...
    },
    {
      ...
      content: [
        { kind: 'paragraph', text: 'Перед тем как отправить счёт клиенту в ЕС, пройдитесь по короткому списку проверок. Это занимает 3 минуты и экономит часы объяснений с бухгалтером и Finanças.' },
        { kind: 'checklist', items: [
